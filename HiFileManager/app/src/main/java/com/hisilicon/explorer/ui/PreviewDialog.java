...
import com.hisilicon.explorer.activity.BaseFileActivity;
...
public class PreviewDialog extends Dialog {
    ...
    private BaseFileActivity mea;
    private Thread thread;
    private ExecutorService exec = Executors.newFixedThreadPool(1);

    public PreviewDialog(Context context, FileInfo fileInfo, BaseFileActivity mea) {
        super(context);
        this.mea = mea;
    ...
    private void initData() {
        ...
        sb.append(getContext().getString(R.string.file_size_1) + FileUtils.convertToHumanReadableSize(mea, fileInfo.getSize())
                + "\n\n");
        String type = fileInfo.getMimeTypes();
        if (type != null && !type.equals("null"))
            sb.append(getContext().getString(R.string.file_type) + type + "\n\n");
        else {
            type = MimeTypeMap.getFileExtensionFromUrl(fileInfo.getPath());
            sb.append(getContext().getString(R.string.file_type) + type + "\n\n");
        }
